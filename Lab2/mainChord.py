...
m_arPos = [int(m) for m in input("Введите идентификатора позиций, в которых находятся узлы: ").split()]  # [0, 1, 3]
...
for i in ChordArray_1.myArray:
    ...
    for k in i.finger:
        print('start: ' + str(k.start[0]) + '; interval: [' + str(k.interval[0]) + ',' + str(
            k.interval[1]) + '); node: ' + str(k.node[0]))
    j += 1
